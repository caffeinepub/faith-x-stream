...
export default function MoviesPage() {
  ...
  const movies = allVideos.filter(
    (v) =>
      !v.isClip &&
      (v.contentType === 'movie' || v.contentType === 'film') &&
      v.availableAsVOD
  );
      ...
      <div className="max-w-screen-2xl mx-auto px-4 py-8">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-foreground/60 text-sm mt-0.5">
                {isLoading ? 'Loading...' : `${movies.length} title${movies.length !== 1 ? 's' : ''} available`}
              </p>
            </div>
          </div>
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-24 text-center">
            <Film className="w-16 h-16 text-foreground/20 mb-4" />
            ...
            <p className="text-foreground/40 text-sm max-w-sm">
              {searchQuery
                ...
                : 'Movies will appear here once they are added to the platform.'}
            </p>
          </div>
        )}
      </div>
